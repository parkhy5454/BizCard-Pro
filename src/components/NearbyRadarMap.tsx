    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">주변 사람 레이더 (가까운 거리순 지도 정렬)</h2>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div 
          ...
          className="lg:col-span-2 bg-white border border-slate-200 rounded-3xl p-6 shadow-xl flex flex-col justify-between aspect-square md:aspect-video relative overflow-visible cursor-default"
        >
              ...
              return (
                ...
                >
                  <div className="relative flex flex-col items-center">
                    ...
                    <div className={`bg-white/95 border px-2 py-1 rounded-lg shadow-xl text-[10px] text-slate-700 whitespace-nowrap transition-colors duration-200 font-medium ${
                      isSelected ? 'border-blue-400 bg-white' : 'border-slate-200 opacity-90 group-hover/pin:opacity-100 group-hover/pin:border-blue-400'
                    }`}>
                      <span className="font-bold text-blue-600">{c.name}</span> <span className="text-slate-400">({c.distanceKm < 1 ? `${Math.round(c.distanceKm * 1000)}m` : `${c.distanceKm.toFixed(1)}km`})</span>
                      {g && <div className="text-[9px] text-amber-600 font-mono">{g.name}</div>}
                    </div>
                    ...
                    {isSelected && (
                      <div 
                        ...
                        className={`absolute z-30 w-64 text-left pointer-events-auto transition-all p-3.5 rounded-2xl bg-white/95 border border-slate-200 shadow-2xl animate-in fade-in zoom-in-95 duration-150 ${
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 border-b border-slate-200 pb-2 mb-2">
                          ...
                          <button 
                            onClick={() => setActiveContactId(null)} 
                            className="text-slate-400 hover:text-slate-800 p-0.5 rounded-lg hover:bg-slate-100 cursor-pointer"
                          >
                            ...
                            <div className="space-y-1.5">
                              ...
                              <div className="grid grid-cols-3 gap-1">
                                <button
                                  ...
                                  className="text-[10px] py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-md border border-amber-200 font-bold transition-all text-center cursor-pointer"
                                  title="티맵 실행"
                                >
                                  티맵
                                </button>
                                <button
                                  ...
                                  className="text-[10px] py-1 bg-yellow-50 hover:bg-yellow-100 text-yellow-700 rounded-md border border-yellow-200 font-bold transition-all text-center cursor-pointer"
                                  title="카카오내비/맵 실행"
                                >
                                  카카오
                                </button>
                                <button
                                  ...
                                  className="text-[10px] py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-md border border-emerald-200 font-bold transition-all text-center cursor-pointer"
                                  title="네이버 지도 실행"
                                >
                                  네이버
                                </button>
                                <button
                                  ...
                                  className="text-[10px] py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-md border border-blue-200 font-bold transition-all text-center cursor-pointer"
                                  title="구글 지도 실행"
                                >
                                  구글맵
                                </button>
                                <button
                                  ...
                                  className="text-[10px] py-1 bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-md border border-purple-200 font-bold transition-all text-center cursor-pointer"
                                  title="애플 지도 실행"
                                >
                                  애플맵
                                </button>
                                <button
                                  ...
                                  className="text-[10px] py-1 bg-slate-800 hover:bg-slate-700 text-white rounded-md border border-slate-800 font-bold transition-all text-center cursor-pointer"
                                  title="명함 상세 보기"
                                >
                                  상세보기
                                </button>
              ...
              filteredAndSortedContacts.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    onClick={() => onSelectContact(c)}
                    className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 hover:border-blue-300 cursor-pointer transition-all flex items-center justify-between gap-3 group"
                  >
