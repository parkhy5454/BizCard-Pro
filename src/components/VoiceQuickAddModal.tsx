  ...
  const handleSave = (asUpdate: boolean) => {
    ...
    if (asUpdate && duplicateMatch && onUpdate) {
      ...
      const newCard: BusinessCard = {
        ...
        // 보이고 특정 그룹엔 안 걸리도록 그룹을 비워둔다 (다른 등록 경로들과 통일).
        // [추가] 사진이 없으니, "가져오기"와 동일하게 이름/회사 정보로 정형화된 명함
        // 이미지를 자동으로 만들어준다. 나중에 실제 명함을 스캔하면 이 이미지가 교체된다.
        frontImage: generateStandardCardImage({ name: parsed.name, company: parsed.company, title: parsed.title, department: parsed.department } as BusinessCard) || undefined,
        isAutoGeneratedImage: true,
