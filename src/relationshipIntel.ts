...
// "관계·영업 인텔리전스" 탭)의 채점/매칭 로직을 한 곳에 모아둔다. 두 화면의 "이 거래처는
// 그만 알려줘"(개별 해제) 목록도 아래 useDismissedContactIds 훅을 통해 같은 저장소를
// 공유하므로, "무엇을 몇 점으로 챙길지 판단하는 기준"과 "해제 여부" 모두 여기서 공용으로
// 관리한다.
//
...
// [수정] 예전에는 CardGrid("전체 명함")와 AIIntelligenceView("AI 인텔리전스" 탭)가 "이 거래처는
// 그만 알려줘"로 개별 해제한 목록을 각각 다른 localStorage 키에 따로 저장했다. 그래서 한쪽
// 화면에서 해제해도 다른 화면에는 반영되지 않아, 은행·보증·보험·컨설팅·인증·연구소·협회처럼
// 원래 영업 대상이 아닌 거래처를 두 화면에서 매번 따로 해제해야 하는 불편이 있었다. 이제는
// 두 화면이 이 훅 하나를 함께 써서 같은 저장소(localStorage 키)를 공유하도록 통일했다 - 둘 중
// 어느 화면에서 해제하든, 그 이후로는 두 화면 모두에서 계속 보이지 않는다.
const DISMISSED_CONTACTS_STORAGE_KEY = 'bizcard_relationship_intel_dismissed_contact_ids';

export function useDismissedContactIds(): {
  dismissedContactIds: string[];
  dismissContact: (contactId: string) => void;
  resetDismissedContacts: () => void;
} {
  ...
  const dismissContact = (contactId: string) => {
    setDismissedContactIds((prev) => {
      ...
      try { localStorage.setItem(DISMISSED_CONTACTS_STORAGE_KEY, JSON.stringify(next)); } catch {}
      return next;
    });
  };

  const resetDismissedContacts = () => {
    try { localStorage.removeItem(DISMISSED_CONTACTS_STORAGE_KEY); } catch {}
    setDismissedContactIds([]);
  };

  return { dismissedContactIds, dismissContact, resetDismissedContacts };
}
